...
void resets_Terminal(int sigNum);
...
int main()
{
	...
	signal(SIGINT, resets_Terminal);	// resets terminal
    	signal(SIGQUIT, SIG_IGN);		// ignores keybod signals
 ...
 */
{
	...
	while ( 1 ){
		...
		if(input == 'y' || input == 'n'){		// checks if input is equal to y or n
